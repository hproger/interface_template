  ...
  exportALL = () => {
    axios
      ...
      .then(({ data }) => {
        ...
        FileSaver.saveAs(
          blob,
          `${curYear}.${curMonth}.${curDay} база номеров.txt`
        );
      })
      .catch(function (error) {
        console.log(error);
      ...
      if (file) {
        ...
        reader.onload = function (evt) {
          axios
            ...
            .then(({ data }) => {
              data.error === ""
                ? alert("Номера импортированы успешно")
      ...
      <div className="page_backups">
        ...
        <div className="row">
          <div className="col-md-12 col-sm-12">
            <div>
              <span className="add_btn" onClick={() => this.exportALL()}>
                <span
                  className="glyphicon glyphicon-tasks"
                  aria-hidden="true"
                ></span>{" "}
                Экспорт базы номеров
              </span>
            </div>
            <div>
              <span className="add_btn" onClick={() => this.importAllMerge()}>
                <span
                  className="glyphicon glyphicon-tasks"
                  aria-hidden="true"
                ></span>{" "}
