...
class Directions extends Component {
    ...
    handleSave = (direction) => {
        if (this.state.status === 1) {
            axios
                .post(routes.trunks.edit, {id: direction.id, name: direction.name, value: direction.value})
                .then(() => {
                    this.getListDirections();
                    this.handleHide();
                })
            ...
            axios
                .post(routes.trunks.add, {name: direction.name, value: direction.value})
                .then(() => {
                ...
                <div className="row">
                    <div className="col-md-12">
                        <span className="add_btn" onClick={() => this.handleAdd()}>
                            <span className="glyphicon glyphicon-plus" aria-hidden="true"></span>&nbsp;Добавить новое направление
                        </span>
                    </div>
                </div>
                <div className="row">
                    ...
                    { this.state.status > 0 &&
                        <div className="col-md-6 col-sm-6">
                            <EditorDirections 
                                title={this.state.status === 1 ? "Редактирование направления" : "Новое направление"} 
                                direction={this.state.status === 1 ? this.state.direction : null}
