...
class EditorNumber extends Component {
    ...
    handleRemovePoolLine = (pool_line_ID) => {
        ...
        const checkLine = this.state.check_line ? _.cloneDeep(this.state.check_line) : [];
    ...
    checkLine = (event) => {
        ...
        let newCheckLine = this.state.check_line ? _.cloneDeep(this.state.check_line) : [];
        if (target.checked) {
                                ...
                                <div className="row">
                                    <div className={this.state.mode ? "col-md-6" : "col-md-12"}>
                                        ...
                                        <div className="form-group">
                                            ...
                                            <input type="text" className="form-control" id="pool_name" name="pool_name" placeholder="Название набора" value={this.state.pool_name} onChange={(event)=>this.handleInputChange(event)} />
                                        </div>
                                        {this.state.pool_tems.map(({type,from,till,digits,country_code, number}, i) => (
                                            <div className="form-group flex-row pool-line" key={i}>
                                                <input type="checkbox" name={'check_line_'+i} data-line={i} onChange={(e)=>this.checkLine(e)} key={i}/>
                                                {type === POOL ? (
                                                    <div className="display_grid grid-area_c2">
                                                        <div className="from_text">с</div><div className="from_value">{from}</div>
                                                        <div className="till_text">по</div><div className="till_value">{till}</div>
                                                        <div className="digits_text">цифр</div><div className="digits_value">{digits}</div>
                                                        <div className="c-code_text">код</div><div className="c-code_value">{country_code}</div>
                                                    </div>
                                                ) : (
                                                ...
                                                <div className="pool-line-btns">
                                                    <button type="button" className="btn btn-default" onClick={() => this.handleEditPoolLine({i, type, from, till, digits, country_code, number})}> 
                                                        <span className="glyphicon glyphicon-edit" aria-hidden="true"></span>
                                                    </button>
                                                    <button type="button" className="btn btn-default" onClick={() => this.handleCopyPoolLine(i)}> 
                                                        <span className="glyphicon glyphicon-copy" aria-hidden="true"></span>
                                                    </button>
                                                    <button type="button" className="btn btn-default" onClick={() => this.handleRemovePoolLine(i)}>
                                        ...
                                        <div className="bottom-left-block btns">
                                            <div><span className="btn add_pool" onClick={()=>this.handleChangeMode(POOL)}>Добавить последовательность</span></div>
                                            <div><span className="btn add_one_number" onClick={()=>this.handleChangeMode(SINGLE)} >Добавить номер</span></div>
                                            <div><span className="btn load_file" onClick={()=>this.handleChangeMode(LOAD_FILE)} >Список из *.csv</span></div>
                                        </div>
                                    </div>
                                    {this.state.mode && (
                                        <div className="col-md-6">
                                        
                                            <div className="edit-pool" style={{ display: this.state.mode === POOL && 'block' }}>
                                                <h4>Добавление последовательности</h4>
                                                <div className="form-group">
                                                    <label htmlFor="from_pool">Начальный номер последовательности:</label>
                                                    <input type="text" className="form-control" id="from_pool" name="from_pool" placeholder="92100" value={this.state.from_pool} onChange={(event)=>this.handleInputChange(event,true)} />
                                                </div>
                                                <div className="form-group">
                                                    <label htmlFor="till_pool">Конечный номер последовательности:</label>
                                                    <input type="text" className="form-control" id="till_pool" name="till_pool" placeholder="92101" value={this.state.till_pool} onChange={(event)=>this.handleInputChange(event,true)} />
                                                </div>
                                                <div className="form-group">
                                                    <label htmlFor="digits">Глубина номера, цифр:</label>
                                                    <input type="text" className="form-control" id="digits" name="digits" placeholder="92100" value={this.state.digits} onChange={(event)=>this.handleInputChange(event,true)} />
                                                </div>
                                                <div className="form-group">
                                                    <label htmlFor="country_code">Код страны:</label>
                                                    <input type="text" className="form-control" id="country_code" name="country_code" placeholder="92100" value={this.state.country_code} onChange={(event)=>this.handleInputChange(event)} />
                                                </div>
                                            ...
                                            <div className="edit-number" style={{ display: this.state.mode === SINGLE && 'block' }}>
                                                ...
                                                <div className="form-group">
                                                    ...
                                                    <input type="text" className="form-control" id="number" name="number" placeholder="+79210900540" value={this.state.number} onChange={(event)=>this.handleInputChange(event)} />
                                                </div>
                                                <div className="form-group edit-btns-wrap">
                                                    <button type="button" className="btn btn-sm btn-default" onClick={()=>this.resetEditLine()}>Отмена</button>
                                            ...
                                            <div className="edit-load_file" style={{ display: this.state.mode === LOAD_FILE && 'block' }}>
                                                ...
                                                <div className="form-group">
                                                    ...
                                                    <input type="file" className="form-control" id="load_file" name="load_file" ref={this.fileInput}  />
                                                </div>
                                                <div className="form-group edit-btns-wrap">
                                                    <button type="button" className="btn btn-sm btn-default" onClick={()=>this.resetEditLine()} >Отмена</button>
                                                    <button type="button" className="btn btn-sm btn-success" onClick={()=>this.generateJsonData(this.state.mode)}>Импортировать</button>
                                                </div>
                                            </div>
